...
def main() -> None:
    ...
    # NOTE: DistilBERT trains on the ~111K-row subset with review text,
    # not the full 231K dataset.  Its test set (~22K rows) differs from
    # the RF test set (46K rows), so metrics are not directly comparable.
    if args.skip_bert:
        ...
        if "combined_reviews" not in features_df.columns:
            ...
            # Evaluate DistilBERT on its own test split (~22K text-only rows)
            bert_preds = trainer.predict(trainer.eval_dataset)
            bert_y_pred = np.argmax(bert_preds.predictions, axis=1)
            bert_y_true = np.array(yl_test)
            bert_report = classification_report(bert_y_true, bert_y_pred, output_dict=True)
            bert_cm = confusion_matrix(bert_y_true, bert_y_pred)

            logger.info("\n=== DistilBERT ===")
            logger.info(classification_report(bert_y_true, bert_y_pred))
            logger.info("Confusion matrix:\n%s", bert_cm)

            OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)
            pd.DataFrame(bert_cm).to_csv(OUTPUTS_DIR / "distilbert_cm.csv")
            results.append({
                "name": "DistilBERT",
                "report": bert_report,
